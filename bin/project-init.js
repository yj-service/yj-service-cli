...
function runSomeScript(progressBar){
    ...
    childProcess.exec('lerna init',(error, stdout, stderr)=>{
        ...
        progressBar.tick({
            token1:"npm install vuepress @commitlint/cli @commitlint/config-conventional husky   cross-env axios\n",
        })
        spinner.text  = '开始下载...\n'; 
        spinner.start();
    }); 
    let p = new Promise((resolve,reject)=>{
        childProcess.exec('npm install  axios -S --registry=https://registry.npm.taobao.org',(error1, stdout1)=>{
            if (error1) {
                reject(error1)
            } 
            console.log(stdout1+'\n\n');
